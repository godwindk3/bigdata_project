...
class LiveTracker:
    def __init__(self, link: str = None, headless: str = True) -> None:
        ...
        self.session_ends_on = datetime.now(
        ) + timedelta(minutes=session_run_minutes) if session_run_minutes > 0 else None
        ...
        self.is_live = True
        self.session_video_info = self.get_video_info(
        ...
        while self.is_tracking:
            with self.page.expect_response("https://www.youtube.com/youtubei/v1/*") as response:
                ...
                if 'live_chat/get_live_chat' in url:
                    actions = data.get('continuationContents').get(
                        'liveChatContinuation').get('actions')
                    if actions is None:
                        continue
                    addChatActions = [
                        item for item in actions if 'addChatItemAction' in item]
                    for chat in addChatActions:
                        ...
                        if liveChatData is None:
                            break
                        message = ' '.join([txt.get('text') or self.__handle_get_emoji(
                            txt) for txt in liveChatData.get('message').get('runs')])
                        author = liveChatData.get(
                            'authorName').get('simpleText')
                ...
                if 'updated_metadata' in url:
                    ...
                    if actions is None:
                        continue
                    viewActions = [
                        item for item in actions if 'updateViewershipAction' in item]
                    for viewEvent in viewActions:
                        one_data = viewEvent.get(
                            'updateViewershipAction').get('viewCount')
        ...
        try:
            ...
            if self.browser == None or self.page == None:
                raise PlaywrightExecutionError(
                    f'Unable to get browser ,Got:{self.browser}')
            ...
            logger.error(
                'Video Link provided is not a live video', stack_info=True)
        except PlaywrightExecutionError:
            print(f'Something went wrong with playwright:{e}')
            logger.error('Something went wrong with Playwright',
                         stack_info=True)
